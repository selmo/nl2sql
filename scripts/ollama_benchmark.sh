...
check_load_time() {
    local file=$1
    # load 시간 파싱 (다양한 형식 지원)
    grep -o "load [0-9.]*ms" "$file" | awk '{print $2}' | sed 's/ms//' | awk '{print $1/1000}' || \
    grep -o "loaded .* in [0-9.]*ms" "$file" | grep -o "[0-9.]*ms" | sed 's/ms//' | awk '{print $1/1000}' || \
    echo "N/A"
}

# 첫 토큰 생성 시간을 확인하는 함수
check_first_token_time() {
    local file=$1
    # first token 시간 파싱 (다양한 형식 지원)
    grep -o "first token [0-9.]*ms" "$file" | awk '{print $3}' | sed 's/ms//' | awk '{print $1/1000}' || \
    grep -o "first token in [0-9.]*ms" "$file" | grep -o "[0-9.]*ms" | sed 's/ms//' | awk '{print $1/1000}' || \
    echo "N/A"
}
...
for CURRENT_MODEL in $MODEL_LIST; do
    ...
    if [ "$WARMUP" = "true" ]; then
        ...
        if [ "$ENV_TYPE" = "docker" ]; then
            docker exec -i "$DOCKER_CONTAINER" ollama run $CURRENT_MODEL "간단한 웜업 프롬프트입니다. 모델을 준비하기 위한 메시지입니다." > /dev/null 2>&1
        else
            ollama run $CURRENT_MODEL "간단한 웜업 프롬프트입니다. 모델을 준비하기 위한 메시지입니다." > /dev/null 2>&1
        fi
    ...
    for i in $(seq 1 $REPEAT); do
        ...
        if [ "$ENV_TYPE" = "docker" ]; then
            docker exec -i "$DOCKER_CONTAINER" ollama run $CURRENT_MODEL "$PROMPT" --verbose > "$OLLAMA_OUTPUT_FILE" 2>&1
        else
            ollama run $CURRENT_MODEL "$PROMPT" --verbose > "$OLLAMA_OUTPUT_FILE" 2>&1
        fi
        ...
        if [ -f "$OLLAMA_OUTPUT_FILE" ]; then
            # 로드 시간 및 첫 토큰 생성 시간 추출 (다양한 형식 지원)
            LOAD_TIME=$(check_load_time "$OLLAMA_OUTPUT_FILE")
            FIRST_TOKEN_TIME=$(check_first_token_time "$OLLAMA_OUTPUT_FILE")

            # 생성된 토큰 수 계산
            TOKENS_GENERATED=$(check_tokens_generated "$OLLAMA_OUTPUT_FILE")
